...
export default async function VisitDetailPage({
  ...
  try {
    visit = await apiFetch<GuestVisit>(`/visits/${visitId}`, token)
  } catch {
    notFound()
  }

  const folio = await apiFetch<Folio>(`/visits/${visitId}/folio`, token).catch(() => null)

  const okMessage = searchParams?.ok
  const errorMessage = searchParams?.error
  const checkoutEligible = ['checked_in', 'active', 'extended'].includes(visit.status)
  const redirectTo = `/visits/${visitId}`
        ...
        <div className="flex gap-2">
          {checkoutEligible && (
            <form action={guestCheckoutAction}>
              ...
              <button className="btn-primary text-xs">Check Out</button>
            </form>
          )}
        ...
        <div className="card p-4">
          ...
          <dl className="space-y-2 text-sm">
            ...
            <div className="flex justify-between items-center">
              ...
              <dd><StatusBadge status={visit.payment_status} /></dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-gray-400">Source</dt>
              <dd className="text-white">{visit.source_type}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-gray-400">Product Type</dt>
              <dd className="text-white">{visit.product_type}</dd>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              <dt className="text-gray-400">Check-in Channel</dt>
              <dd className="text-white">{visit.check_in_channel ?? '—'}</dd>
            </div>
          </dl>
        </div>
        <div className="card p-4">
          ...
          <dl className="space-y-2 text-sm">
            ...
            {visit.booking_id && (
              <div className="flex justify-between">
                ...
                <dd className="text-xs font-mono text-gray-300">{visit.booking_id.slice(0, 8)}…</dd>
              </div>
            )}
            {visit.assigned_resource_id && (
              <div className="flex justify-between">
                <dt className="text-gray-400">Resource</dt>
                <dd className="text-xs font-mono text-gray-300">{visit.assigned_resource_id.slice(0, 8)}…</dd>
              </div>
            )}
            <div className="flex justify-between">
              <dt className="text-gray-400">Started</dt>
              <dd className="text-xs text-white">
                {visit.start_time ? new Date(visit.start_time).toLocaleString() : '—'}
              </dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-gray-400">Ended</dt>
              <dd className="text-xs text-white">
                {visit.actual_end_time ? new Date(visit.actual_end_time).toLocaleString() : '—'}
              </dd>
            </div>
          </dl>
        </div>
        ...
        <div className="card overflow-hidden mb-6">
          <div className="px-4 py-3 border-b border-gray-700 flex items-center justify-between">
            ...
            <div className="text-xs text-gray-400">
              Balance:{' '}
              <span className={folio.balance_due_cents > 0 ? 'text-red-400 font-semibold' : 'text-green-400 font-semibold'}>
                ${(folio.balance_due_cents / 100).toFixed(2)}
              </span>
              ...
              {folio.line_items.length === 0 ? (
                ...
                folio.line_items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-700/40">
                    ...
                    <td className="px-4 py-2 text-xs text-white text-right">
          ...
          {folio.balance_due_cents > 0 && (
            <div className="border-t border-gray-700 p-4">
              ...
              <form action={recordFolioPaymentAction} className="flex flex-wrap gap-2">
                ...
                <input
                  ...
                  defaultValue={folio.balance_due_cents}
                  className="form-input text-xs w-40"
                  required
                />
                <button className="btn-primary text-xs">Record Payment</button>
              </form>
            </div>
          )}
          ...
          <div className="border-t border-gray-700 p-4">
            ...
            <form action={addFolioLineItemAction} className="flex flex-wrap gap-2">
              <input type="hidden" name="folioId" value={folio.id} />
              <input type="hidden" name="redirectTo" value={redirectTo} />
