...
export function FloorPlansClient({ token }: { token: string }) {
  ...
  if (loading) return <div className="max-w-5xl"><h1 className="text-2xl font-semibold mb-6 text-text-primary">Floor Plans</h1><p className="text-text-muted">Loading…</p></div>

  return (
    <div className="max-w-5xl">
      <h1 className="text-2xl font-semibold mb-6 text-text-primary">Floor Plans</h1>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {plans.length === 0 ? (
          <div className="card p-6 text-sm text-text-muted">No floor plans found.</div>
        ) : (
          plans.map((plan) => (
            <div key={plan.id} className="card p-4">
              ...
              <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mb-3">
                <dt className="text-text-muted">Areas</dt>
                <dd className="text-text-primary">{plan.areas.length}</dd>
                <dt className="text-text-muted">Location ID</dt>
                <dd className="font-mono text-xs text-text-muted break-all">{plan.locationId}</dd>
              </dl>
