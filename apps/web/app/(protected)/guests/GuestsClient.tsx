...
export function GuestsClient({ token }: { token: string }) {
  ...
  return (
    <div className="max-w-5xl">
      ...
      {createError && (
        <div className="mb-4 rounded-md border border-red-700 bg-red-900 text-red-200 px-3 py-2 text-sm">{createError}</div>
      )}
        ...
        <div className="card overflow-hidden mb-6">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-700">
              {searchError ? (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-red-400">{searchError}</td></tr>
              ) : guests.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-500">No guests found.</td></tr>
              ) : guests.map((guest) => (
                <tr key={guest.id} className="hover:bg-gray-700/40">
                  <td className="px-4 py-3 text-white font-medium">{guest.firstName} {guest.lastName ?? ''}</td>
