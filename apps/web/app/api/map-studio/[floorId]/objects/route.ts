...
export async function GET(_req: NextRequest, { params }: { params: { floorId: string } }) {
  ...
  const res = await fetch(`${API_BASE}/map-studio/floors/${params.floorId}/objects`, { headers: headers(session.accessToken), cache: 'no-store' })
...
export async function PUT(req: NextRequest, { params }: { params: { floorId: string } }) {
  ...
  const res = await fetch(`${API_BASE}/map-studio/floors/${params.floorId}/objects`, {
    method: 'PUT',
