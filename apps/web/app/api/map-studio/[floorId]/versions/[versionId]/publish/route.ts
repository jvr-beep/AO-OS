...
export async function PUT(_req: NextRequest, { params }: { params: { floorId: string; versionId: string } }) {
  const session = await getSession()
  if (!session.accessToken) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const res = await fetch(`${API_BASE}/map-studio/floors/${params.floorId}/versions/${params.versionId}/publish`, {
    method: 'PUT',
